...
import {
  ...
  getAllBudgets,
  upsertBudget,
  toggleStar,        // ✅ correct name from db.ts
  deleteTransaction,
  getUser,           // ✅ for user currency
  Transaction,
} from "../../utils/db";
...
// ── currency code → symbol map (matches profile.tsx) ──────────────────────
...
export default function AnalyticsScreen() {
  ...
  const [currency, setCurrency]         = useState("$"); // ✅ user-defined, default until loaded
  ...
  const loadData = useCallback(() => {
    ...
    // ✅ load user's selected currency symbol
    const users = getUser();
  ...
  const handleStar = useCallback((t: Transaction) => {
    toggleStar(t.id, t.starred === 1); // ✅ correct signature
    loadData();
  }, [loadData]);
      ...
      <View style={styles.periodRow}>
        {(["week", "month", "quarter", "year"] as TimePeriod[]).map((p) => (
          <PeriodTab
            ...
            active={period === p} onPress={setPeriod} C={C}
          />
        ))}
      </View>
